...
@php
    $heads = [['label' => __('Código'), 'width' => 10], __('Nome'), __('Marca'), __('Categoria'), ['label' => __('Ativo'), 'width' => 10], ['label' => __('Ações'), 'no-export' => true, 'width' => 5]];
    ...
    foreach ($products as $product) {
        ...
        $productsData[] = [$product->id, $product->name, $product->productBrand->name ?? '-', $product->productCategory->name ?? '-', $product->active ? '<span class="badge badge-success">Ativo</span>' : '<span class="badge badge-danger">Desativado</span>', '<nobr>' . $btnShow . $btnEdit . '</nobr>'];
    }

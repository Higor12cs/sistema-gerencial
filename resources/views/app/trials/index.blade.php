...
@php
    $heads = [['label' => __('Código'), 'width' => 10], __('Cliente'), __('Data'), __('Data Retorno'), __('Valor Total'), ['label' => __('Status'), 'width' => 10], ['label' => __('Ações'), 'no-export' => true, 'width' => 5]];
    ...
    foreach ($trials as $trial) {
        ...
        $trialsData[] = [$trial->id, $trial->customer->name, $trial->date->format('d/m/Y'), $trial->return_date ? $trial->return_date->format('d/m/Y') : '-', number_format($trial->total_price / 100, 2, ',', '.'), 'Aguardando', '<nobr>' . $btnShow . $btnEdit . '</nobr>'];
    }
